/**
 * AgentMobileMenu — Full-screen mobile navigation overlay for the agent portal.
 * Mirrors the AdminMobileMenu pattern: portaled to document.body, slides down
 * from top with spring animation, all agent sidebar sections with collapsible
 * subsections, live badge counts, 48px+ touch targets, active route highlighting,
 * profile header with logout, quick-action row (dark mode, language, help),
 * and a search/filter feature for quick navigation.
 */
...
export function AgentMobileMenu({ isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Update timer every second while open
    const interval = setInterval(() => {
      setTimeRemaining(AccessCodeService.getAgentTimeRemaining());
    }, 1000);
    return () => clearInterval(interval);
  }, [isOpen, session?.agentId]);
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              <div
                ...
                onClick={() => handleNav("/agent/profile")}
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/20 flex-shrink-0 overflow-hidden">
                  {agentAvatar ? (
                    <img src={agentAvatar} alt={session?.agentName || "Agent"} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-white font-bold text-sm">
                      {(session?.agentName || "A").charAt(0).toUpperCase()}
                    </span>
                  )}
                </div>
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/5">
              ...
              <button
                onClick={() => {
                  toast.info(
                    isUrdu
                      ? "سپورٹ سے رابطہ کریں: +92 318 6986259"
                      : "Contact support: +92 318 6986259"
                  );
                  onClose();
                }}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 active:bg-white/10 transition-colors min-h-[40px]"
            ...
            {/* Session & Quick Stats Card */}
            <div className="px-4 pt-2.5 pb-1">
              {/* Session Timer + Check-in Row */}
              <div className="flex items-stretch gap-2 mb-2">
                ...
                <div className={`flex-1 flex items-center gap-2.5 px-3 py-2.5 rounded-xl border ${
                  (() => {
                    ...
                    if (hoursLeft < 0.5) return "bg-red-500/10 border-red-500/20";
                    if (hoursLeft < 1) return "bg-amber-500/10 border-amber-500/20";
                  ...
                  <Timer className={`w-4 h-4 flex-shrink-0 ${
                    (() => {
                      const hoursLeft = timeRemaining / (1000 * 60 * 60);
                      if (hoursLeft < 0.5) return "text-red-400";
                      if (hoursLeft < 1) return "text-amber-400";
                      return "text-blue-400/70";
                    })()
                  }`} />
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase tracking-wider text-blue-300/50 font-medium leading-tight">
                      {isUrdu ? "سیشن" : "Session"}
                    </p>
                    <p className={`text-sm font-mono font-bold tabular-nums leading-tight ${
                      ...
                      {AccessCodeService.formatTimeRemaining(timeRemaining)}
                    </p>
                  </div>
                </div>
                ...
                <motion.button
                  ...
                  onClick={() => {
                    if (isCheckedIn) {
                      toast.info(isUrdu ? "آج آپ پہلے ہی چیک ان ہو چکے ہیں" : "Already checked in today");
                      return;
                    }
                    if (session?.agentId && session?.agentName) {
                      try {
                        const record = AttendanceService.checkIn(session.agentId, session.agentName);
                        setIsCheckedIn(true);
                        if (record.status === "late") {
                          toast.warning(isUrdu ? `تاخیر سے چیک ان - ${record.checkIn}` : `Late check-in at ${record.checkIn}`);
                        } else {
                          toast.success(isUrdu ? `چیک ان کامیاب - ${record.checkIn}` : `Checked in at ${record.checkIn}`);
                        }
                      } catch { toast.error(isUrdu ? "چیک ان میں خرابی" : "Check-in failed"); }
                    }
                  }}
                  className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl border min-h-[48px] transition-colors ${
                    isCheckedIn
                      ? "bg-green-500/10 border-green-500/20 text-green-400"
                      : "bg-blue-500/15 border-blue-500/25 text-blue-300 active:bg-blue-500/25"
                  }`}
                >
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1" style={{ WebkitOverflowScrolling: "touch" }}>
                ...
                <motion.button
                  ...
                  onClick={() => handleNav("/agent/cases")}
                  className="flex-shrink-0 flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/5 border border-white/8 active:bg-white/10 transition-colors min-w-[110px]"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-blue-300/50 font-medium leading-tight">{isUrdu ? "کل" : "Total"}</p>
                  </div>
                </motion.button>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-lg font-bold text-green-400 leading-tight">{completedCount}</p>
                    <p className="text-[10px] text-blue-300/50 font-medium leading-tight">{isUrdu ? "مکمل" : "Done"}</p>
                  </div>
                </motion.button>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-lg font-bold text-orange-400 leading-tight">{pendingCount}</p>
                    <p className="text-[10px] text-blue-300/50 font-medium leading-tight">{isUrdu ? "زیر التوا" : "Active"}</p>
                  </div>
                </motion.button>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-[11px] font-semibold text-green-300 leading-tight">{isUrdu ? "نیا کوڈ" : "New Code"}</p>
                              ...
                              {section.subsections!.map((sub, subIdx) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {sub.badge && (
                                      <span
                                        className={`min-w-[18px] h-[18px] px-1 rounded-full ${sub.badgeColor || "bg-blue-500"} text-white text-[9px] font-bold flex items-center justify-center`}
                                      >
                                        {sub.badge}
                                      </span>
                                    )}
            ...
            >
              ...
              <div className="flex items-center justify-center gap-1.5 py-1">
                ...
                <span className="text-[10px] tracking-wider font-medium text-blue-500/40">
                  {isUrdu ? "ایمرالڈ CRM v2.0" : "EMERALD CRM v2.0"}
                </span>
                <Sparkles className="w-3 h-3 text-blue-500/40" />
              </div>
